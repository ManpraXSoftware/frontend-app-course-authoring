...
export async function getStudioHomeCourses(search) {
  const { data } = await getAuthenticatedHttpClient().get(`${getApiBaseUrl()}/api/courses${search}`);
  return camelCaseObject(data);
}
/**
...
export async function getTaxonomies() {
  const { data } = await getAuthenticatedHttpClient().get(`${getConfig().STUDIO_BASE_URL}/api/content_tagging/v1/taxonomies/`);
  return camelCaseObject(data);
}

export async function getContentTypes({ id, tags_count }) {
  const { data } = await getAuthenticatedHttpClient().get(`${getApiBaseUrl()}/api/content_tagging/v1/taxonomies/${taxonomyId}/tags/?full_depth_threshold=${depthThreshold}`);
  return camelCaseObject(data);
}