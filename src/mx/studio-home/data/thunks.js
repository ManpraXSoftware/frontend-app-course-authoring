...
function fetchTaxonomiesData() {
  return async (dispatch) => {
    try {
      const tagsData = await getTaxonomies();
      dispatch(fetchTaxonomiesDataSuccess(tagsData.results));
      dispatch(updateLoadingStatuses({ tagsLoadingStatus: RequestStatus.SUCCESSFUL }));
    } catch (error) {
      dispatch(updateLoadingStatuses({ tagsLoadingStatus: RequestStatus.FAILED }));
      return;
    }
...
function fetchContentTypesData() {
  return async (dispatch) => {
    try {
      const tagsData = await getContentTypes();
      dispatch(fetchContentTypesDataSuccess(tagsData.results));
      dispatch(updateLoadingStatuses({ ContentTypesLoadingStatus: RequestStatus.SUCCESSFUL }));
    } catch (error) {
      dispatch(updateLoadingStatuses({ ContentTypesLoadingStatus: RequestStatus.FAILED }));
      return;
    }
