  ...
  return (
    <div className="library-stepper">
      {steps.length ? steps.map(({ title, description }, index) => {
        ...
        return (
          <div
            className={classNames('library-stepper__step', {
              active: isActiveClass,
              done: isDoneClass,
              error: isErrorClass,
            })}
            key={title}
            data-testid="library-stepper__step"
          >
            <div className="library-stepper__step-icon">
              <Icon src={stepIcon} alt={title} data-testid={`${title}-icon`} />
            </div>
            <div className="library-stepper__step-info">
              <h3 className="h4 title library-stepper__step-title font-weight-600">{title}</h3>
              {isPercentShow && (
                <p
                  className="library-stepper__step-percent font-weight-400"
                  data-testid="library-stepper__step-percent"
                >
                  {percent}%
                </p>
              )}
              <p className="library-stepper__step-description font-weight-400">
...
LibraryStepper.defaultProps = {
  percent: false,
  hasError: false,
  errorMessage: '',
};

LibraryStepper.propTypes = {
...
export default injectIntl(LibraryStepper);
