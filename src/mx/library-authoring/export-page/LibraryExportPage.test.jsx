...
import { getExportStatusApiUrl, postExportLibraryApiUrl } from './data/api';
import { EXPORT_STAGES } from './data/constants';
import { exportPageMock } from './__mocks__';
import messages from './messages';
import LibraryExportPage from './LibraryExportPage';

let store;
let axiosMock;
let cookies;
const libraryId = '123';
const libraryName = 'About Node JS';
...
const RootWrapper = () => (
  <AppProvider store={store}>
    <IntlProvider locale="en" messages={{}}>
      <LibraryExportPage intl={injectIntl} libraryId={libraryId} />
    </IntlProvider>
  </AppProvider>
);

describe('<LibraryExportPage />', () => {
  beforeEach(() => {
    initializeMockApp({
      authenticatedUser: {
        userId: 3,
    ...
    axiosMock
      .onGet(postExportLibraryApiUrl(libraryId))
  ...
  it('should render page title correctly', async () => {
    ...
    await waitFor(() => {
      ...
      expect(helmet.title).toEqual(
        `${messages.headingTitle.defaultMessage} | ${libraryName} | ${process.env.SITE_NAME}`,
      );
    });
  });
  ...
  it('should show modal error', async () => {
    axiosMock
      .onGet(getExportStatusApiUrl(libraryId))
  ...
  it('should show download path for relative path', async () => {
    ...
    // eslint-disable-next-line no-promise-executor-return
    await new Promise((r) => setTimeout(r, 3500));
    const downloadButton = getByText(stepperMessages.downloadLibraryButtonTitle.defaultMessage);
