...
import { startExportingLibrary } from './data/thunks';
...
const LibraryExportPage = ({ intl, libraryId }) => {
  const dispatch = useDispatch();
  const exportTriggered = useSelector(getExportTriggered);
  const libraryDetails = useModel('courseDetails', libraryId);
      ...
      <Helmet>
        <title>
          {intl.formatMessage(messages.pageTitle, {
            ...
            libraryName: libraryDetails?.name,
          ...
          >
            <Layout.Element>
              <article>
                  ...
                  {isShowExportButton && (
                    <Card.Section className="px-3 py-1">
                      <Button
                        ...
                        block
                        className="mb-4"
                        onClick={() => dispatch(startExportingLibrary(libraryId))}
                ...
                {exportTriggered && <ExportStepper libraryId={libraryId} />}
                <ExportFooter />
              </article>
            </Layout.Element>
            <Layout.Element>
              <ExportSidebar libraryId={libraryId} />
            </Layout.Element>
          </Layout>
        </section>
        <ExportModalError libraryId={libraryId} />
      </Container>
...
LibraryExportPage.propTypes = {
  intl: intlShape.isRequired,
  libraryId: PropTypes.string.isRequired,
};

LibraryExportPage.defaultProps = {};

export default injectIntl(LibraryExportPage);
