...
import {
  startLibraryExporting,
  getLibraryExportStatus,
} from './api';
import {
...
export function startExportingLibrary(libraryId) {
  return async (dispatch) => {
    dispatch(updateSavingStatus({ status: RequestStatus.PENDING }));
    try {
      dispatch(reset());
      dispatch(updateExportTriggered(true));
      const exportData = await startLibraryExporting(libraryId);
...
export function fetchExportStatus(libraryId) {
  return async (dispatch) => {
    dispatch(updateLoadingStatus({ status: RequestStatus.IN_PROGRESS }));
    try {
      const {
        exportStatus, exportOutput, exportError,
      } = await getLibraryExportStatus(libraryId);
