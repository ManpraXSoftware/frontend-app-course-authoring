...
const ExportStepper = ({ intl, libraryId }) => {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      if (isStopFetching) {
        ...
        dispatch(fetchExportStatus(libraryId));
      }
    }, 3000);
  ...
  return (
    <div>
      ...
      />
      {downloadPath && currentStage === EXPORT_STAGES.SUCCESS && <Button className="ml-5.5 mt-n2.5" href={downloadPath} download>{intl.formatMessage(messages.downloadLibraryButtonTitle)}</Button>}
    </div>
  );
};
