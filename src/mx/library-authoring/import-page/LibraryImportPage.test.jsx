...
import { getLibraryImportStatusApiUrl } from './data/api';
...
const RootWrapper = () => (
  <AppProvider store={store}>
    <IntlProvider locale="en" messages={{}}>
      <LibraryImportPage intl={injectIntl} libraryId={libraryId} />
    </IntlProvider>
  </AppProvider>
);
...
describe('<LibraryImportPage />', () => {
  beforeEach(() => {
    ...
    axiosMock
      .onGet(getLibraryImportStatusApiUrl(libraryId, 'testFileName.test'))
  ...
  it('should show error', async () => {
    axiosMock
      .onGet(getLibraryImportStatusApiUrl(libraryId, 'testFileName.csv'))
      .reply(200, { importStatus: -IMPORT_STAGES.UPDATING, message: '' });
    cookies.get.mockReturnValue({ date: 1679787000, completed: false, fileName: 'testFileName.csv' });
    const { getByText } = render(<RootWrapper />);
