...
import { getImportStatus, startLibraryImporting } from './api';
import {
...
import { IMPORT_STAGES, LAST_LIBRARY_IMPORT_COOKIE_NAME } from './constants';

export function fetchImportStatus(libraryId, fileName) {
  return async (dispatch) => {
    try {
      dispatch(updateLoadingStatus(RequestStatus.IN_PROGRESS));
      const { importStatus, message } = await getImportStatus(libraryId, fileName);
      dispatch(updateCurrentStage(Math.abs(importStatus)));
      const cookies = new Cookies();
      const cookieData = cookies.get(LAST_LIBRARY_IMPORT_COOKIE_NAME);
...
export function handleProcessUpload(libraryId, fileData, requestConfig, handleError) {
  return async (dispatch) => {
    try {
